...
empleo <- subset(data_final, trabsp == "Sí")
...
Extra 
Analisis bivariado con las otras variables 
```{r}
library(dplyr)
library(ggplot2)

# Filtrar solo casos completos (sin NA en las variables de interés)
biv_data <- empleo_mod %>%
...
# 1) Acceso a internet (interh)
tabla_internet <- table(biv_data$trabajo_formal_v2, biv_data$interh)
prop.table(tabla_internet, margin = 2) * 100  # Porcentaje dentro de cada nivel de interh

# 2) Dominio geográfico
tabla_dominio <- table(biv_data$trabajo_formal_v2, biv_data$dominio)
prop.table(tabla_dominio, margin = 2) * 100

# 3) Sexo
tabla_sexo <- table(biv_data$trabajo_formal_v2, biv_data$sexo_d)
prop.table(tabla_sexo, margin = 2) * 100

# 4) Estado civil
tabla_estciv <- table(biv_data$trabajo_formal_v2, biv_data$estciv_ord)
prop.table(tabla_estciv, margin = 2) * 100

# 5) Lengua materna
tabla_lengua <- table(biv_data$trabajo_formal_v2, biv_data$lengua_cat)
prop.table(tabla_lengua, margin = 2) * 100

# 6) Último año de estudio
tabla_ultani <- table(biv_data$trabajo_formal_v2, biv_data$ultani_cat)
prop.table(tabla_ultani, margin = 2) * 100
...
# 1) Dominio geográfico: volver categórica con etiquetas
...
# 2) Sexo: dicotómica (1 mujer, 0 hombre)
empleo_mod <- empleo_mod %>%
  mutate(
...
# 3) Estado civil: categórica ordinal, unir viudo y divorciado
empleo_mod <- empleo_mod %>%
  mutate(
...
# 4) Lengua materna: categórica ordinal, cambios especiales
empleo_mod <- empleo_mod %>%
  mutate(
...
# 5) Último año de estudio: categórica ordinal con etiquetas y ajustes
empleo_mod <- empleo_mod %>%
  mutate(
    ultani_cat = case_when(
      ultani == 1 ~ "Sin nivel",
      ultani %in% c(2,12) ~ "Educación Inicial",
    ...
    ultani_cat = factor(ultani_cat,
                        levels = c("Sin nivel","Educación Inicial","Primaria incompleta",
...
Se aplica cuando las variables son categóricas o dicotómicas.

Encuentra dimensiones que resumen las asociaciones entre categorías de varias variables.

Ideal para tu caso, porque la mayoría de tus variables (sexo_d, estciv_ord, dominio, lengua_cat, ultani_cat) son categóricas.

💡 Conclusión: En tu dataset, MCA es el más apropiado.
...
gráfico de individuos y de categorías
